...
public class KafkaMessageAdapter implements JsonSerializer<KafkaMessage>, JsonDeserializer<KafkaMessage> {
    ...
    @Override
    public JsonElement serialize(KafkaMessage src, Type typeOfSrc, JsonSerializationContext context) {
        JsonObject json = new JsonObject();
        json.addProperty("action", src.action().name());

        if (src.message() instanceof Long) {
            json.addProperty("content", (Long) src.message());
        } else if (src.message() instanceof byte[]) {
            String base64Content = Base64.getEncoder().encodeToString((byte[]) src.message());
            json.addProperty("content", base64Content);
        } else {
            json.add("content", context.serialize(src.message()));
        }
